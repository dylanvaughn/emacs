...
(defconst feature-mode-keywords
  '("Feature" "Background", "Scenario", "Given", "Then", "When", "And"))
...
(defconst feature-font-lock-keywords
  (list
   ...
   '("^ *Background:" (0 font-lock-keyword-face) (".*" nil nil (0 font-lock-function-name-face t)))
  ...
  (if (bobp)
    ...
    (let ((not-indented t) cur-indent)
      ...
      (save-excursion
        (while not-indented
          (forward-line -1)
          (if (or (looking-at "^[ \t]*Scenario:") (looking-at "^[ \t]*Background:"))
              (progn
                (setq not-indented nil)
